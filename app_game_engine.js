...
function loadRosterPage() {
  ...
  confirmButton.addEventListener('click', () => {
    ...
    let hTeam = leagueTeamList.find(
      (item) => item.tID === gameTeams[0].dataset.tid
    );
    let aTeam = leagueTeamList.find(
      (item) => item.tID === gameTeams[1].dataset.tid
    );
    ...
    gmArray.forEach((item) => {
      if (item.lastElementChild.dataset.pid) {
        freeAgentRoster.find(
          (slot) => slot.pID === Number(item.lastElementChild.dataset.pid)
        ).benchStatus = item.dataset.benchStatus;
        homeTeamRoster[0][item.dataset.pos] = freeAgentRoster.find(
          (slot) => slot.pID === Number(item.lastElementChild.dataset.pid)
        );
      } else {
        homeTeamRoster[0][item.dataset.pos] = {
          pID: 1001,
          firstName: '',
          lastName: '',
          position: '',
    ...
    cpuArray.forEach((item) => {
      if (item.lastElementChild.dataset.pid) {
        ...
        awayTeamRoster[0][item.dataset.pos] = freeAgentRoster.find(
          (slot) => slot.pID === Number(item.lastElementChild.dataset.pid)
        );
      } else {
        awayTeamRoster[0][item.dataset.pos] = {
          pID: 1002,
          firstName: '',
          lastName: '',
          position: '',
...
function launchSim() {
  ...
  boxscoreTiles.forEach((tile) => {
    tile.addEventListener('mouseover', () => {
      ...
      tile.lastElementChild.firstElementChild.style.height = `${
        tile.childNodes[5].lastElementChild.innerText * 0.2
      }px`;
      if (
        tile.childNodes[5].lastElementChild.innerText < 75 &&
        tile.childNodes[5].lastElementChild.innerText > 49
      ) {
        tile.lastElementChild.firstElementChild.style.backgroundColor =
          'orange';
  ...
  function postPlayerData() {
    ...
    homePosList.forEach((item) => {
      //Post Player Name
      item[0].innerText = `${homeTeam.roster[0][item[1]].firstName.charAt(
        0
      )}. ${homeTeam.roster[0][item[1]].lastName}`;
      // Post Player Stats
      item[0].nextElementSibling
        .querySelectorAll('.stat-box')
        .forEach((stat) => {
          stat.innerText =
            homeTeam.roster[0][item[1]].boxscore[stat.dataset.statCat];
        });
    ...
    awayPosList.forEach((item) => {
      //Post Player Name
      item[0].innerText = `${awayTeam.roster[0][item[1]].firstName.charAt(
        0
      )}. ${awayTeam.roster[0][item[1]].lastName}`;
      // Post Player Stats
      item[0].nextElementSibling
        .querySelectorAll('.stat-box')
        .forEach((stat) => {
          stat.innerText =
            awayTeam.roster[0][item[1]].boxscore[stat.dataset.statCat];
        });
    ...
    hFG.innerText = `${Math.round(
      (sumTeamStats(homeTeam, homePosList, 'FG') /
        sumTeamStats(homeTeam, homePosList, 'FGA')) *
        100
    )}%`;
    hTPS.innerText = `${Math.round(
      (sumTeamStats(homeTeam, homePosList, 'TP') /
        sumTeamStats(homeTeam, homePosList, 'TPA')) *
        100
    )}%`;
    ...
    aFG.innerText = `${Math.round(
      (sumTeamStats(awayTeam, awayPosList, 'FG') /
        sumTeamStats(awayTeam, awayPosList, 'FGA')) *
        100
    )}%`;
    aTPS.innerText = `${Math.round(
      (sumTeamStats(awayTeam, awayPosList, 'TP') /
        sumTeamStats(awayTeam, awayPosList, 'TPA')) *
        100
    )}%`;
  }
  ...
  function simPos(homeTeam, roadTeam) {
    ...
    function takeShot(shooter, shot_selection) {
      ...
      function madeShotPhrase() {
        switch (Math.floor(Math.random() * 7 + 1)) {
          ...
          case 5:
            ...
            break;
          case 6:
            return 'with a high-arching rainbow... He connects on';
            break;
          case 7:
            return 'drains';
            break;
      ...
      function shotTypePhrase(x) {
        switch (x) {
          case 'pRat_closeShot':
            switch (Math.floor(Math.random() * 3 + 1)) {
              case 1:
                ...
                break;
              case 2:
                return 'the shot after driving the lane and finishing with a lay-up';
                break;
              case 3:
                return 'an easy bucket at the rim';
                break;
              default:
                '4th option';
            }
          ...
          case 'pRat_midShot':
            switch (Math.floor(Math.random() * 3 + 1)) {
              case 1:
                return 'a jumper from the wing';
                break;
              case 2:
                return 'a mid-range shot';
                break;
              case 3:
                return 'a well-contested, fall-away jumper';
                break;
              default:
                'a 20-footer';
            }
          ...
          case 'pRat_threeShot':
            switch (Math.floor(Math.random() * 3 + 1)) {
              case 1:
                return 'a jumper from beyond the arc';
                break;
              case 2:
                return 'a 40-footer from the logo';
                break;
              case 3:
      ...
      switch (shot_selection) {
        case 'pRat_closeShot':
          defAttribute = defender.pRat_intDef;
          break;
        case 'pRat_midShot':
          defAttribute = defender.pRat_perDef;
          break;
        default:
          defAttribute = defender.pRat_perDef;
      }
      ...
      if (shooter[shot_selection] >= rand + defMultiplier) {
        shotDescription = `${
          shooter.lastName
        } ${madeShotPhrase()} ${shotTypePhrase(shot_selection)}`;
        shooter.boxscore.FG++;
    ...
    function setPriority(team, attribute) {
      let playerAttRatings = [
        team.roster[0].pg[attribute],
        team.roster[0].sg[attribute],
        team.roster[0].sf[attribute],
        team.roster[0].pf[attribute],
        team.roster[0].c[attribute],
      ];
      ...
      let weightedPercent = weightedRatings.map(
        (item) => item / weightedRatings.reduce((a, b) => a + b)
      );
      ...
      let distributionArray = [
        [pgPriority, team.roster[0].pg],
        [sgPriority, team.roster[0].sg],
        [sfPriority, team.roster[0].sf],
        [pfPriority, team.roster[0].pf],
        [cPriority, team.roster[0].c],
      ].sort((a, b) => b[0] - a[0]);
    ...
    switch (Math.floor(Math.random() * 3 + 1)) {
      case 1:
        shotSelection = 'pRat_closeShot';
        pointValue = 2;
        break;
      case 2:
        shotSelection = 'pRat_midShot';
        pointValue = 2;
        break;
      default:
        shotSelection = 'pRat_threeShot';
        pointValue = 3;
    }
    ...
    if (Math.random() <= 0.09) {
      ...
      console.log(
        `Pos ${posCount}: STEAL!!! The ball is stolen by ${stealer.lastName}.`
      );
      console.log('');
    } else {
      ...
      if (shotOutcome === 'Make') {
        ...
        switch (Math.floor(Math.random() * 5 + 1)) {
          case 1:
            defender = defense.roster[0].pg;
            break;
          case 2:
            defender = defense.roster[0].sg;
            break;
          case 3:
            defender = defense.roster[0].sf;
            break;
          case 4:
            defender = defense.roster[0].pf;
            break;
          default:
            defender = defense.roster[0].c;
        }
        //defensive outcome------------------------------------------------------------
      ...
      console.log(
        `Pos ${posCount}: ${shotDescription}${assistOutcome}. ${defensiveOutcome}`
      );
...
function loadPlayerGenerator() {
  ...
  submitButton.addEventListener('click', () => {
    ...
    let playerExp =
      document.querySelectorAll('.active')[1].dataset.exp === 'rookie'
        ? 0
        : Math.floor(Math.random() * (15 - 1) + 1);
    let team = leagueTeamList.find(
      (team) => team.tID === document.querySelectorAll('.active')[2].dataset.tid
    );
  ...
  saveButton.addEventListener('click', () => {
    ...
    console.log(leagueTeamList[0].roster);
  ...
  counters.forEach((counter) => {
    counter.addEventListener('click', (event) => {
      ...
      switch (event.target.classList[0]) {
        case 'minus':
          counter.childNodes[3].value > counter.childNodes[3].min
            ? counter.childNodes[3].value--
            : (counter.childNodes[3].value = counter.childNodes[3].min);
          ...
          break;
        case 'plus':
          counter.childNodes[3].value === counter.childNodes[3].max
            ? ''
            : counter.childNodes[3].value++;
